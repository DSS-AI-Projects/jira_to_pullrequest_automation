...
async def fresh_gitlab_access_token(
    connection: RepoHostingConnection, store: JobStore, settings: Settings
) -> str:
    """The decrypted access token for a stored GitLab connection, refreshed
    first if it's stale. Raises AppError when it can't be decrypted or
    refreshed. Scope policy (which operations the token may be used for)
    lives in app/auth/git_auth.py, not here."""
    _connection, access_token = await _get_valid_access_token(connection, store, settings)
